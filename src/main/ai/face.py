...
@router.post("/age", status_code=status.HTTP_201_CREATED, response_model=Age)
def age(image: bytes = File(), user_id: str = Depends(auth)):
    ...
    # Response model
    model_name = 'age'
    age = predict_json(project, region, model_name, request_data, version_v1)

    # Age Predictions
    labels = age[0]['labels']
    predictions_age = labels[np.argmax(age[0]['scores'])]

    return {'age': predictions_age}
