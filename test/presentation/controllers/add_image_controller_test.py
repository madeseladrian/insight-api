...
from ..mocks.glasses import AddImageSpy
from ..mocks.validation import ValidationSpy


class TestAddImageController:
    # SetUp
    faker = Faker()
    ...
    def make_sut(self) -> SutTypes:
        add_image_spy = AddImageSpy()
        validation_spy = ValidationSpy()
        sut = AddImageController(
            add_image=add_image_spy,
            validation=validation_spy
        )

        return sut, add_image_spy, validation_spy
    ...
    def test_4_should_call_AddImage_with_correct_values(self):
